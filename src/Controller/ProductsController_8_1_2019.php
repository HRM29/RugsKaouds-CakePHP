...
class ProductsController extends AppController {
	...
	public function productView($product_id = null) {
		...
		$productDetail	=	$this->Products->find('all')->where(['sku_no'=>$sku])->first();
		...
		$featuredProductData = $ProductsTable->find('all')->where(['Products.is_future' => 1,'sku_no !='=>$sku])->toArray();
	...
	public function cart() {
		...
		$this->set(compact('cardData','paypalemail','userData')); 
	}
	...
	public function getFilterParam(){
		...
		if(!empty($data)){  
			...
			if(!empty($data['collection'])){
				foreach($data['collection'] as $key =>$dataVal){
					if($dataVal != '0'){
			...
			if(!empty($keyArr)){
				...
				if(!empty($arrParms)){
					...
					if(!empty($slug_v)){
						...
						/* //
						if(!empty($styl_arr)){
							$filters['OR'][]['Products.category_id IN'] = $styl_arr;
						...
						//print_r($col_arr);  die;
						if(!empty($col_arr)){
						    //$filters['OR'][]['Products.color_id IN'] = $col_arr;
						    foreach($col_arr as $clkey => $colVal){ 
						        $filters['OR']['AND'][$clkey]['Products.other_colors LIKE'] = "%$colVal%";
    						   
						    }
						    
						    //$filters['OR'][]['Products.color_id IN'] =
						    
						}
						
						$filters['OR'][]['Products.sku_no LIKE'] = "%$slug_v%";
						$filters['OR'][]['Products.title LIKE'] = "%$slug_v%";
