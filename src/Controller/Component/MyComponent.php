 ...
 */
    ...
    public function customerList()
    {
        ...
        ])->where(['status' => 1, 'role_id' => 3,])->order(['first_name' => 'ASC']);
        $customer = $query->all();
    ...
    public function BrandList()
    {
        ...
        $query = $Table->find('list', [
            'keyField' => 'id',
            'valueField' => 'title'
        ])->where(['status' => 1])->order(['title' => 'ASC']);
        $brands = $query->all();
    ...
    public function SubCategoryList($cat_id)
    {
        if (!empty($cat_id)) {
            ...
            $query = $Table->find('list', [
                'keyField' => 'id',
                'valueField' => 'title'
            ])->where(['status' => 1, 'parent_id' => $cat_id])->order(['title' => 'ASC']);
    ...
    public function processMediaUpload($file, $folder_name, $folder_slug = null)
    {
        if (!empty($file['name'])) {
            $uploadPath = WWW_ROOT . 'uploads' . DS . $folder_name . DS;
            ...
            if (move_uploaded_file($file['tmp_name'], $filePath)) {
                // Generate media URL
                $mediaUrl = Router::url('/', true) . 'media/view/' . $folder_slug ?? $folder_name . '/' . $customName;
