...
class ProjectsController extends AppController
{
	...
	{
		$title = 'Projects';
		$table = TableRegistry::getTableLocator()->get('Projects');
		...
		if ($this->request->is(['post', 'put'])) {
			...
			if (!$data->getErrors()) {
				if (empty($this->request->getData()['image']['name'])) {
					...
					$img = $this->My->uploadfile($imageData, 'projects');
					$data->image_url = $img;
				}
				if ($Table->save($data)) {
					$this->Flash->set('Project added successfully.', ['key' => 'positive', 'params' => ['class' => 'alert alert-success']]);
	...
	public function edit($id = null)
	{
		...
		if ($this->request->is(['post', 'put'])) {
			...
			if (!$data->getErrors()) {
				if (empty($this->request->getData()['image']['name'])) {
					...
					if (!empty($image_file)) {
						...
						if (in_array($ext, $image_ext)) {
							...
							$img = $this->My->uploadfile($image_file, 'projects');
				...
				if ($Table->save($data)) {
					$this->Flash->set('Project updated successfully.', ['key' => 'positive', 'params' => ['class' => 'alert alert-success']]);
					$this->redirect(array('controller' => 'Projects', 'action' => 'index'));
				}
			} else {
				$this->Flash->set($this->errorMessage($data->getErrors()), ['key' => 'positive', 'params' => ['class' => 'alert alert-danger']]);
	...
	public function delete($id = null)
	{
		...
		if ($Table->delete($data)) {
			...
			if (!empty($data->image_url)) {
				$original = WWW_ROOT . 'uploads' . DS . 'projects' . DS . $data->image_url;
				if (file_exists($original)) {
					unlink($original);
				}

				$original_thumb = WWW_ROOT . 'uploads' . DS . 'projects/thumb' . DS . $data->image_url;
	...
	public function deleteImg()
	{
		...
		$project_image = $this->Projects->get($id);


		$removeImage = $project_image[$fieldName];

		$original = WWW_ROOT . 'uploads' . DS . 'projects' . DS . $removeImage;

		if (file_exists($original)) {
			unlink($original);
		}
		$original_thumb = WWW_ROOT . 'uploads' . DS . 'projects/thumb' . DS . $removeImage;
		if (file_exists($original_thumb)) {
			unlink($original_thumb);
		}

		$project_image[$fieldName] = '';

		if ($this->Projects->save($project_image)) {
			$result =  1;
		}
		echo $result;
	}
