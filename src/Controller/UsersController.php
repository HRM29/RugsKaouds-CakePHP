...
class UsersController extends AppController
{
	...
	{
		parent::initialize();
		$this->Auth->allow(['logout', 'index', 'signup', 'forgotPassword', 'resetPassword', 'login', 'recaptcha', 'gLogin', 'socialFacebook', 'createusername', 'makesession', 'activate', 'addToCart', 'myAccountRedirect']);
	}
	public function myAccountRedirect()
	{
		// Check if the user is logged in
		if ($this->Auth->user()) {
			// Redirect to the myaccount action if logged in
			return $this->redirect(Router::url('/my-account-2', true));
		} else {
			// Redirect to the login action if not logged in
			return $this->redirect(Router::url('/my-account', true));
		}
	}
	public function register()
	{
		...
		if ($this->request->is(['post', 'put'])) {
			...
			$postdata['password'] = "HbV5o_>M@01(5;DL>my6";
			$postdata['confirm_password'] = "HbV5o_>M@01(5;DL>my6";
	...
	public function login()
	{
		...
		if ($this->request->is('post')) {
			...
			if (empty($user)) {
				...
				if (!empty($this->request->getData('remember-me-check')) && $this->request->getData('remember-me-check') == 1) {
					// Encrypt the user ID for security
					$encryptedUserId = Security::encrypt($user['id'], Security::getSalt());
					...
					$this->response = $this->response->withCookie('RememberMe', [
						...
						'expire' => (new FrozenTime())->addDays(5)->toUnixString(),
					]);
				}
	...
	public function forgotPassword()
	{
		...
		if ($this->request->is(['post', 'put'])) {
			...
			if (!empty($results)) {
				...
				$base64Encoded = base64_encode($results->id);
				$hmacKey = KAOUDS_SALT;
				$hmac = hash_hmac('sha256', $base64Encoded, $hmacKey);

				$activation_link = Router::url('/', true) . 'users/resetPassword/' . $base64Encoded . '/' . $hmac;
				$EmailTemplates = TableRegistry::getTableLocator()->get('EmailTemplates');
	...
	public function resetPassword($id = null, $hmac = null)
	{
		if ($this->request->is(['post'])) {
			$id	=	str_replace('==', '', $id);
			$id	=	base64_decode($id);
			...
			if ($this->request->getData()['password'] == $this->request->getData()['password2']) {
				...
				$query->execute();
				$this->Flash->set('Password Updated successfully.', ['key' => 'positive_login', 'params' => ['class' => 'alert alert-success']]);
			...
			$hmacKey = KAOUDS_SALT;
			$calculatedHmac = hash_hmac('sha256', $id, $hmacKey);
			if (hash_equals($hmac, $calculatedHmac)) {
				...
				$this->Flash->set('Invalid URL.', ['key' => 'positive_login', 'params' => ['class' => 'alert alert-danger']]);
				$this->redirect(['controller' => 'users', 'action' => 'login']);
			}
		}
	}
	...
	public function activate($id = null)
	{
		...
		$hmacKey = KAOUDS_SALT;
		$hmac = hash_hmac('sha256', $id, $hmacKey);

		$decodedID = base64_decode($id);
		$searchResult = $usersTable->find('all')->Where(['id' => $decodedID])->first();
		if (!empty($searchResult)) {
			...
			$query->update()
				...
				->where(['id' => $decodedID])
				->execute();
			$this->Flash->set('Account has been activated successfully, please set a strong password.', ['key' => 'positive_reset', 'params' => ['class' => 'alert alert-success']]);
			return $this->redirect(Router::url('/', true) . 'users/resetPassword/' . $id . '/' . $hmac);
		} else {
			$this->Flash->set('Account does not exist.', ['key' => 'positive_login', 'params' => ['class' => 'alert alert-danger']]);
			return $this->redirect(array('controller' => 'users', 'action' => 'login'));
		}
	}
