		...
		<div class="row">
									...
									<tr>
										...
										<td>
											<div class="qnty">
												...
												<input type="number" name="<?= $data['id']; ?>_qnty" class="number" id="<?= $data['id']; ?>_number" value="<?= $data['product_qty']; ?>" />
			...
			<div class="col-md-4">
				<div class="table-responsive crt_sbttl">
					...
					<div class="cupn">
						<h3><img src="/Kaouds/img/cpn.png" alt="coupon"> Coupon</h3>
						<div class="coupon-input-wrapper">

							<input class="fotm_control" type="text" name="coupn-code" value="<?= $couponCode; ?>" placeholder="Coupon Code">
							<?php if ($session->check('coupon')): ?>
								<a href="javascript:void(0);" class="btn remove" id="remove-coupon">&times;</a>
							<?php endif; ?>
						</div>
						<a href="javascript:void(0);" class="btn aply" id="apply-coupon">Apply Coupon</a>
						<ul>
...
<script>
	...
	function applyCoupon() {
		var couponCode = $("input[name='coupn-code']").val().trim();
		if (couponCode === "") {
			$("input[name='coupn-code']").css('border', '2px solid red').focus();
			$(".coupon-error").remove(); // Remove any existing error message
			$("input[name='coupn-code']").after('<div class="coupon-error" style="color: red; margin-top: -5px;margin-bottom: 15px">Coupon code cannot be empty.</div>');
			return;
		} else {
			$("input[name='coupn-code']").css('border', '');
