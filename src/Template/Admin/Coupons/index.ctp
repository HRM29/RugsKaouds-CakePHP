			...
			<div class="box box-primary">
				...
				<div class="box-body">
					...
					<div class="col-sm-3">
						...
						<?= $this->Form->control('code', ['placeholder' => 'Code', 'label' => false, 'class' => 'form-control', 'value' => isset($savesearch['code']) ? $savesearch['code'] : '', 'required' => false]); ?>
					</div>
					<div class="col-sm-3">
					...
					<div class="col-sm-3">
						...
						<a href="<?= Router::url('/', true) . '/admin/coupons/clearSearch'; ?>" class="btn btn-primary clear">Clear</a>
					</div>
				</div><!-- /.box-body -->
		...
		<div class="col-xs-12">
			<div class="box">
				<div class="box-header">
					...
					<div class="box-tools">
						...
						echo $this->Html->link(
							...
							array('escape' => false, 'class' => "btn btn-success btn-sm", "title" => __("Add Coupon", true))
						);
						?>
						<?php
				...
				<div class="box-body table-responsive no-padding">
					<?php echo $this->Form->create('', ['url' => ['action' => 'deleteAllCoupons'], 'id' => 'listForm']); ?>
					<?php echo $this->Form->end() ?>
					<table class="table table-hover">
						<tr>
							...
							<th><?= $this->Paginator->sort('User By') ?></th>
							<th><?= $this->Paginator->sort('Type') ?></th>
							<th><?= $this->Paginator->sort('discount') ?></th>
							<th><?= $this->Paginator->sort('From/TO') ?></th>
						...
						<?php foreach ($data as $key => $page): ?>
							<tr>
								...
								<td><?= h($page->title) ?></td>
								<td><?= h($page->code) ?></td>
								<td><?= $this->General->getUsers($page->user_id) ?></td>
								<td><?= $this->General->getChargeType($page->type) ?></td>
								<td><?= h(number_format($page->discount, 2)) ?></td>
								<td><?= h(date('d-m-Y', strtotime($page->start_date)) . ' to ' . date('d-m-Y', strtotime($page->valid_date))) ?></td>
								<td><?= $this->General->getAdminStatus($page->status) ?></td>
								<td><?= h(date('d-m-Y H:i:s', strtotime($page->created))) ?></td>
								<td class="actions">
									<?php
									echo $this->Html->link(
...
<script>
	...
	$(".btnDeleteAll").click(function() {
		...
		if (favorite.length < 1) {
			...
			if (confirm('Are you sure you want to delete the selected Coupon?')) {
				if (favorite.length > 0) {

					var url = '<?php echo Router::url('/', true) . 'admin/Coupons/deleteAllCoupons'; ?>';
