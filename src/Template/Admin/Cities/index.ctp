			...
			<div class="box box-primary">
				...
				<div class="box-body">
					<div class="col-sm-3">
						...
						<?php  
						echo $this->Form->control('con_id', ['options' => $country,'label'=>false,'class'=>'form-control country','value'=>isset($savesearch['con_id'])?$savesearch['con_id']:'','empty'=>'Select Country']);
						?>
                    </div>
					<div class="col-sm-3">
						<label for="status">States</label>
						<?php  
						echo $this->Form->control('sta_id', ['label'=>false,'class'=>'form-control state','value'=>isset($savesearch['sta_id'])?$savesearch['sta_id']:'','empty'=>'Select States']);
						?>
                    </div>
					<div class="col-sm-3">
							<label for="Fund">City Name</label>
							<?= $this->Form->control('title', ['placeholder'=>'City Name','label'=>false,'class'=>'form-control','value'=>isset($savesearch['title'])?$savesearch['title']:'','required' => false]);?>
					</div> 
					<div class="col-sm-3">
					...
					<div class="col-sm-3">
						...
						<a href="<?= Router::url('/', true).'/admin/cities/clearSearch';?>" class="btn btn-primary clear">Clear</a>
					</div> 
                  </div><!-- /.box-body --> 
		...
		<div class="col-xs-12"> 
		  <div class="box">
			<div class="box-header">
			  ...
			  <div class="box-tools">
					<?php 
					echo $this->Html->link('<i class="fa fa-plus"></i> Add City',
					array('controller' => 'cities','action'=>'add'),
					array('escape' => false,'class'=>"btn btn-success btn-sm","title"=>__("Add City",true))
					);
					?> 
					<?php 
					/*echo $this->Html->link('<i class="fa fa-trash"></i> Delete All',
					'#',
					array('escape' => false,'class'=>"btn btn-danger btn-sm btnDeleteAll","title"=>__("Delete City",true))
					);*/
					?> 
			...
			<div class="box-body table-responsive no-padding">
				...
				<tr>
					...
					<td><?= $this->General->getCountry($page->con_id) ?></td>
					<td><?= $this->General->getStates($page->sta_id) ?></td>
					<td><?= h($page->name) ?></td> 
					<td><?= $this->General->getAdminStatus($page->status) ?></td>
					<td class="actions">
						<?php 
							echo $this->Html->link('<i class="fa fa-eye"></i> View',
								array('controller' => 'cities','action'=>'view', base64_encode($page->id)),
								array('escape' => false,'class'=>"btn btn-primary btn-sm","title"=>__("View",true)));
						?>
						<?php 
							echo $this->Html->link('<i class="fa fa-pencil"></i> Edit',
								array('controller' => 'cities','action'=>'edit', base64_encode($page->id)),
								array('escape' => false,'class'=>"btn btn-info btn-sm","title"=>__("Edit",true)));
						?>
						<?php 
							/*echo $this->Html->link('<i class="fa fa-trash"></i> Delete',
								array('controller' => 'cities','action'=>'delete', base64_encode($page->id)),
		...
		if(favorite.length < 1){
			...
			if(confirm('Are you sure you want to delete the selected city?')){
				if(favorite.length > 0){
					...
					var url 	= '<?php echo Router::url('/', true).'/admin/cities/deleteAllCities';?>';
