				    ...
				    <div class="form-group row">
						...
						<div class="col-xs-5">
							  ...
							  <div class="controls">
								<?php
									if (!empty($data->image)) {
										$original = WWW_ROOT . 'uploads/brands/thumb' . DS . $data->image;
										if (file_exists($original)) { ?>
											<?php 
												echo $this->Html->image('../uploads/brands/thumb/' . $data->image, array('width' => '100px', 'class' => 'img-responsive imgbdr remove_image'));
...
<script>
	...
	$('.remove_image').on('click', function(){
		...
		if(confirm('Are you sure Remove Brands Image?'))
		{
			$.ajax({
				dataType: "html",
				type: "POST",
				evalScripts: true,
				url: '<?php echo Router::url(['controller'=>'brands','action'=>'deleteImg']);?>',
