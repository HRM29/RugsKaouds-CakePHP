		...
		<div class="col-xs-12">
			<div class="box box-primary">
				<div class="box-header with-border">
					<h3 class="box-title">Search Notification</h3>
                </div>
				<?php echo $this->Form->create($latestnews,array(
				...
				<div class="box-body">
                    ...
                    <div class="col-sm-3">
						<label for="return_time">Start Time</label>
						<?= $this->Form->control('return_time', ['placeholder'=>'Start Time','label'=>false,'class'=>'form-control','value'=>isset($savesearch['return_time'])?$savesearch['return_time']:'','required' => false]);?>
                    </div> 
						<div class="col-sm-3">
							<label for="Fund">End Time</label>
							<?= $this->Form->control('fund', ['placeholder'=>'End Time','label'=>false,'class'=>'form-control','value'=>isset($savesearch['fund'])?$savesearch['fund']:'','required' => false]);?>
						</div> 
		...
		<div class="col-xs-12"> 
		  <div class="box">
			<div class="box-header">
			  ...
			  <div class="box-tools">
					<?php 
					echo $this->Html->link('<i class="fa fa-plus"></i> Add Notification',
					array('controller' => 'LatestNews','action'=>'add'),
					array('escape' => false,'class'=>"btn btn-success btn-sm","title"=>__("Add Notification",true))
					);
					?>
                     
					<?php 
					echo $this->Html->link('<i class="fa fa-trash"></i> Delete All',
					array('controller' => 'LatestNews','action'=>'deleteAllProjects'),
					array('escape' => false,'class'=>"btn btn-danger btn-sm btnDeleteAll","title"=>__("Delete Project",true))
					);  
					?> 
			...
			<div class="box-body table-responsive no-padding">
			  <?php echo $this->Form->create('',['url'=>['action'=>'deleteAllProjects'],'id'=>'listForm']); ?>
			  <table class="table table-hover"> 
				<tr>
					...
					<th><?= $this->Paginator->sort('Date Range') ?></th>
					<th><?= $this->Paginator->sort('status') ?></th>
					<th><?= $this->Paginator->sort('created') ?></th>
					<th class="actions"><?= __('Actions') ?></th>
				</tr>
				<?php $i=0;foreach ($latestnews as $dataArr): $i++;?>
		
				<tr>
					<td class="a-center ">
						<?php echo $this->Form->control('project_chk.',array('type'=>"checkbox","label"=>false,'div'=>false,'class'=>"status_checkbox tableflat",'value'=>$dataArr->id)) ; ?>
					</td>
					
					<td><?= $i ?></td> 
					<td><?= $dataArr->title ?></td>
					<td><?= $dataArr->date_range ?></td>
					<td><?= $this->General->getStatus($dataArr->status) ?></td>
					<td><?= $dataArr->created ?></td> 
					<td class="actions">
						<?php 
							/* echo $this->Html->link('<i class="fa fa-eye"></i> View',
								array('controller' => 'LatestNews','action'=>'view', $dataArr->id),
								array('escape' => false,'class'=>"btn btn-primary btn-sm","title"=>__("View",true))); */
						?>
						<?php 
							echo $this->Html->link('<i class="fa fa-pencil"></i> Edit',
								array('controller' => 'LatestNews','action'=>'edit', $dataArr->id),
								array('escape' => false,'class'=>"btn btn-info btn-sm","title"=>__("Edit",true)));
						?>
						<?php 
							echo $this->Html->link('<i class="fa fa-trash"></i> Delete',
								array('controller' => 'LatestNews','action'=>'delete', $dataArr->id),
								array('escape' => false,'class'=>"btn btn-danger btn-sm","title"=>__("Delete",true),'confirm' => __('Are you sure you want to delete # {0}?', $dataArr->id)));
						?> 
					</td> 
				</tr>
				<?php endforeach; ?> 
