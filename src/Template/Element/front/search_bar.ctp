...
<div class="col-lg-3 col-md-4 sidebar-filter"><span class="filter-by">Filter by</span>
	...
	<?php echo $this->Form->create('rugs',['id'=>'CustomerRugsForm', 'class'=>'ajaxform registerForm','novalidate'=>false,'url'=>['controller'=>'products','action'=>'searchProducts']]);
		...
		//$req_url = ($this->request->getPath()) ? $this->request->getPath() : '';
  ...
  <a href="javascript:void(0)" class="closebtn" onclick="closeNav0()">&times;</a>
	...
	<nav class="nav" role="navigation">
		<ul class="nav__list filter_custom"> 
			<li>
				<input id="group-4" type="checkbox" <?php if(!empty($style_val)){echo "checked";}?> hidden />
			...
			?>
				<input id="group-1" type="checkbox" <?php if(!empty($activetype)){echo "checked";}?> hidden />
				...
				//if(!empty($allSizes)) {
					...
					foreach($options as $opt_key => $optsval){
						...
						//echo "<pre>";print_r($allSizes);die;
				?>
					<li> 
						<input id="sub-group-<?= $opt_key ?>" type="checkbox" <?php if (in_array($opt_key, $activetype)){ echo "checked";}?> hidden />
						...
						<ul class="sub-group-list">
							...
							?>
								<li class="row sidebarDropdown">
									...
									<div class="col-lg-5">
									<?php  
										/* $speceialSizeHeight_drop = Configure::read('speceialSizeHeight_drop');
										echo $this->Form->control('speceial_size_height', ['type'=>'select','options' => $speceialSizeHeight_drop,'label'=>false,'class'=>'form-control','value'=>$speceial_size_height_value,'empty'=>'Select Height']); */
			...
			<li> 
				<input id="group-2" type="checkbox" <?php if(!empty($color_val)){echo "checked";} ?> hidden />
			...
			<li> 
				<input id="group-5" type="checkbox" <?php if(!empty($pattern_val)){echo "checked";} ?> hidden />
			...
			<li> 
				<input id="group-6" type="checkbox" <?php if(!empty($design_val)){echo "checked";} ?> hidden />
			...
			<li> 
				<input id="group-7" type="checkbox" <?php if(!empty($material_val)){echo "checked";} ?> hidden />
			...
			<li> 
				<input id="group-8" type="checkbox" <?php if(!empty($construction_val)){echo "checked";} ?> hidden />
			...
			<li>  
				<input id="group-3" type="checkbox"  <?php 	if(!empty($price_val) || !empty($price_sort_val)){echo "checked";}?> hidden />
...
<style>
/* reset css */ 
.nav a, .nav label {display: block; padding: .85rem; color: #000; background-color: #FFF; box-shadow: inset 0 -1px #1d1d1d; -webkit-transition: all .25s ease-in; transition: all .25s ease-in;}

.nav a:focus, .nav a:hover, .nav label:focus, .nav label:hover {background: #151567;}
...
.group-list a, .group-list label { padding-left: 2rem; background: #FFF; box-shadow: inset 0 -1px #373737;}
...
.sub-group-list a, .sub-group-list label {padding-left: 4rem;background: #FFF;box-shadow: inset 0 -1px #474747;}
...
.sub-sub-group-list a, .sub-sub-group-list label {padding-left: 3rem; background: #FFF;text-align:left;box-shadow: inset 0 -1px #575757;}
 ...
 * Hide nested lists
 */
.group-list, .sub-group-list, .sub-sub-group-list {height: 100%;max-height: 0;overflow: hidden;-webkit-transition: max-height .5s ease-in-out;transition: max-height .5s ease-in-out;margin: 0;padding: 0;}

.sub-group-list, .sub-sub-group-list {position: relative;/*height: 140px;*/max-height:0px;-webkit-transition: max-height .5s ease-in-out;transition: max-height .5s ease-in-out;overflow-x: hidden;overflow-y: scroll;width: 100%;border-left: 1px solid #eee;border-bottom: 1px solid #eee;margin-top: 0 !important;}

.nav__list input[type=checkbox]:checked + label + ul { /* reset the height when checkbox is checked */
max-height: 1000px; margin: 11px 0;}
...
.sidebarDropdown select{font-size:12px;}
</style>
